...
class KuaishouPlatform(BasePlatform):
    ...
    使用 OBS WebSocket API 控制推流，配置参数与抖音类似但服务器地址不同
    """
    ...
    def _validate_config(self):
        ...
        if not self._rtmp_url:
            raise PlatformError("快手推流 URL 不能为空", "CONFIG_ERROR")
        
        if not self._stream_key:
            raise PlatformError("快手推流密钥不能为空", "CONFIG_ERROR")
    ...
    def start_stream(self, rtmp_url: Optional[str] = None, 
        ...
        try:
            ...
            logger.info(f"快手直播已启动：{self._rtmp_url}")
            
            return True
            
        except Exception as e:
            logger.error(f"启动快手直播失败：{e}")
